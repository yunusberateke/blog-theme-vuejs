      ...
      <v-card-text style="min-height: 85vh">
        <v-form>
          <v-card width="200" height="200">
            <v-img width="200" height="200" :src="this.formData.image"></v-img>
          </v-card>
          <v-file-input
            v-model="image"
...
export default {
      ...
      if (this.image) {
        ...
        serverClient
          ...
          .then((result) => {
            ...
            this.$store
              .dispatch("PostModule/update", this.formData)
              .then((e) => {
          ...
          .then((e) => {
            this.chips.forEach((element) => {
              this.$store
                ...
                .then(() => {
                  this.$toast.success(`Başarıyla Tag Eklendi ${element.name}`, {
                    top: true,
                    right: true,
                  });
                });
  ...
  mounted() {
    ...
    Promise.all([
      ...
      this.$store.dispatch("PostModule/findById", {
        id: this.$route.params.id,
      }),
      this.$store.dispatch("TagPostModule/findByPostId", {
        postId: this.$route.params.id,
      }),
    ]).then((e) => {
      this.content = e[2].content;
      this.formData = e[2];
      this.chips = e[3];
